  ...
  std::shared_ptr<DynamixelSDKWrapper> & dxl_sdk_wrapper)
{
  ...
  {dxl_sdk_wrapper->get_data_from_device<int32_t>(
    ...
    dxl_sdk_wrapper->get_data_from_device<int32_t>(
      extern_control_table.present_position_right.addr,
      extern_control_table.present_position_right.length),dxl_sdk_wrapper->get_data_from_device<int32_t>(
        extern_control_table.present_position_lift.addr,   // 새로 추가
        extern_control_table.present_position_lift.length)};
  ...
  {dxl_sdk_wrapper->get_data_from_device<int32_t>(
    ...
    dxl_sdk_wrapper->get_data_from_device<int32_t>(
      extern_control_table.present_velocity_lift.addr,   // 새로 추가
      extern_control_table.present_velocity_lift.length)};
  ...
  msg->name.push_back(wheel_lift_joint_); // 새로 추가
  ...
  msg->velocity.push_back(RPM_TO_MS * velocity[2]); // 새로 추가
  ...
  last_diff_position[2] += (position[2] - last_position[2]); // 새로 추가
